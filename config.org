...
(use-package eat
  :config
  ;; Add M-o to the list of keys handled by Emacs instead of the terminal
  (add-to-list 'eat-semi-char-non-bound-keys [?\e ?o])
  ;; Update the keymap to apply the change immediately
...
;; Use eglot + pyright for richer completions and signatures
...
(use-package eca
  ...
  :config
  ;; Enable inline AI completion in programming buffers.
